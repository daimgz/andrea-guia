...
def parse_output_aliases(output_aliases: str, outputs: list[str]) -> dict[str, str]:
    ...
    # 2. Limpiar el string completo de saltos de línea y comillas
    clean_aliases = output_aliases.replace('\n', '').replace('"', '').strip()

    # 3. Dividir por comas
    alias_pairs = [pair.strip() for pair in clean_aliases.split(",") if ":" in pair]
    ...
    for pair in alias_pairs:
        try:
            ...
            # Buscamos una coincidencia parcial o exacta
            # Esto ayuda si el nombre real es ligeramente distinto (p.ej. 1f.3 vs lf.3)
            for real_output in outputs:
                if key in real_output or real_output in key:
                    alias_dict[real_output] = value
        except:
            continue
